<?php
/*
Plugin Name: Twitter Tweets for RoloPress
Plugin URI: http://rolopress.com/plugins/twitter-tweets-for-rolopress
Description: Display Contact and Company Tweets from Twitter. | REQUIREMENTS: <a href="http://rolopress.com/themes/rolopress-core">RoloPress Core Theme</a>.
Version: 1.0.1
Author: SlipFire LLC.
 ...
 * @Plugin: Wickett Twitter Widget
 * @URI: http://wordpress.org/extend/plugins/wickett-twitter-widget
 * @Desc: Display Tweets in the sidebar of your blog
 * @Version Number: 1.0.1
 * @Contributor: Automattic Inc.
 * @Contributor URI: http://automattic.com/
*/
...
function wpcom_time_since( $original, $do_more = 0 ) {
  ...
  function rolo_contact_tweet($contact_id) { // get the contact tweet id
      if (!$contact_id) {
      return false;
      }
      ...
      return $contact['rolo_contact_twitter'];
      } 
	  
  function rolo_company_tweet($company_id) { // get the company tweet id
	  if (!$company_id) {
      return false;
    }
    ...
    $company = $company[0];
      ...
      return $company['rolo_company_twitter'];
      } 
	  

class Rolo_Twitter_Tweets extends WP_Widget {

	function Rolo_Twitter_Tweets() {
		$widget_ops = array('classname' => 'rolo_twitter_widget', 'description' => __( "Display Contact and Company tweets from Twitter") );
		$this->WP_Widget('twitter', __('Twitter Tweets'), $widget_ops);
	}

	function widget( $args, $instance ) {
		if (is_single() ) { // only display on single pages
		extract( $args );
				if ( rolo_type_is( 'contact' ) )  $account = urlencode( rolo_contact_tweet(get_the_ID())) ;
				if ( rolo_type_is( 'company' ) )  $account = urlencode( rolo_company_tweet(get_the_ID())) ;
	...
	function _rolo_twitter_widget_username( $matches ) { // $matches has already been through wp_specialchars
		return "$matches[1]@<a href='" . clean_url( 'http://twitter.com/' . urlencode( $matches[2] ) ) . "'>$matches[2]</a>";
	}

	function _rolo_twitter_widget_hashtag( $matches ) { // $matches has already been through wp_specialchars
		return "$matches[1]<a href='" . clean_url( 'http://search.twitter.com/search?q=%23' . urlencode( $matches[2] ) ) . "'>#$matches[2]</a>";
	}

}

add_action( 'widgets_init', 'rolo_twitter_tweets_init' );
function rolo_twitter_tweets_init() {
	register_widget('Rolo_Twitter_Tweets');
}